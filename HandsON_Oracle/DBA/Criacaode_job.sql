...
- Para o nao periodico é só nao passar o terceiro parametro
...
1.Se os parâmetros abaixo estão configurados no arquivo de configuração do banco de dados (init.ora), caso eles não 
estejam configurados, segue abaixo uma sugestão de configuração: 

job_queue_processes = 2 
job_queue_interval = 60 

2.Também deve-se verificar se o pacote DBMS_JOB está criado no banco de dados. Para criá-lo deve-se executar o script 
...
como TRUE, então a procedure terá seu “parse” durante a primeira execução do job; 
...
No exemplo acima a procedure está executando um job chamado TESTE001. O job de número 5 será executado a 
cada 60 segundos. 
...
DBMS_JOB.BROKEN =>Procedure usada para habilitar/desabilitar o “flag” de broken. Quando este flag é habilitado 
o job não é re-executado automaticamente. 
...
Este artigo descreveu como as procedures da package DBMS_JOB podem ajudar o usuário a criar, modificar, 
remover os jobs em sua aplicação. 
